...
def autolog(message):
    info = inspect.currentframe().f_back.f_code
    logging.debug('{0}:{1}'.format(info.co_name, message))
...
class ESILVM(object):
    ...
    def get_reg(self, name):
        ...
        # return  z3.simplify(get_(self.registers[key]))
    ...
    def read_mem(self, addr, size):
        ...
        if key not in self.memory:
            # self.memory[key] = self.__new_mem_var()
    ...
    def __set(self, dst, x):
        if self.__is_register(dst):
            self.set_reg(dst, x)
        elif self.__is_x86_flag(dst):
            pass # we ignore x86 flags for now
        elif self.__is_ESIL_flag(dst):
            pass # we ignore ESIL flags for now
        else:
        ...
        for op in expr.split(','):
            ...
            if self.__is_register(op)  or self.__is_x86_flag(op) \
               ...
               or self.__is_segment_register(op) or self.__is_eflags(op):
                ...
                self.push(self.__new_unk_var())
            else:
                x = to_int(op)
                self.push(z3.BitVecVal(x, 32))
